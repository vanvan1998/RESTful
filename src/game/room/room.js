...
const createNewRoom = async (io, socket) => {
  ...
  socket.adapter.rooms[socket.socketRoomName].namePlayer1 =
    socket.socketUserName;
  socket.adapter.rooms[socket.socketRoomName].imagePlayer1 =
    socket.socketUserImage;
  ...
  socket.emit('server-send-new-message', {
    message: 'Please waiting other player...',
    owner: 'server'
  });
};
...
const joinRandomRoom = async (io, socket) => {
  ...
  for (r in io.sockets.adapter.rooms) {
    ...
    if (r.split('-')[0] === 'room' && clientNumber === 1) {
      ...
      socket.adapter.rooms[socket.socketRoomName].idPlayer2 =
        socket.socketUserId;
      socket.adapter.rooms[socket.socketRoomName].namePlayer2 =
        socket.socketUserName;
      socket.adapter.rooms[socket.socketRoomName].imagePlayer2 =
        socket.socketUserImage;
      socket.join(r);
      break;
    }
  ...
  if (found) {
    var room = await Room.findById(
      socket.adapter.rooms[socket.socketRoomName].roomId
    );
